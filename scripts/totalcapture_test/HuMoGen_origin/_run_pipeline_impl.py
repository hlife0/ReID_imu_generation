...
def _load_humogen_module(module_name: str, module_path: Path):
    spec = importlib.util.spec_from_file_location(module_name, module_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not load module {module_name} from {module_path}")
    module = importlib.util.module_from_spec(spec)
...
_humogen_synthesize = _load_humogen_module("humogen_synthesize_imu", Path("/data/luoyizhang/HuMoGen/src/core/synthesize_imu.py"))

enforce_quaternion_continuity = _repo_adapter.enforce_quaternion_continuity
load_imu_csv = _repo_evaluation.load_imu_csv
synthesize_imu = _humogen_synthesize.synthesize_imu
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the HuMoGen-origin synthetic IMU pipeline on the current processed triplet.")
    parser.add_argument("--processed-root", default="data/processed")
    parser.add_argument("--sequence-name", default="S1_freestyle3")
    parser.add_argument("--sensor-name", default=TARGET_SENSOR)
    parser.add_argument("--output-root", default="outputs/totalcapture_test/HuMoGen_origin")
...
def load_generated_smplx_stream(smplx_npz: Path, sensor_name: str) -> dict[str, np.ndarray]:
    ...
    positions, quaternions_wxyz = compute_sensor_trajectory(joints, placement=placement)
    ...
    imu = synthesize_imu(
        ...
        quat_world_in=quaternions_xyzw.astype(np.float32),
        target_hz=float(fps),
        add_noise=True,
    ...
    quat = enforce_quaternion_continuity(quaternions_wxyz.astype(np.float64))
    zeros_mag = np.zeros((len(imu["accel_x"]), 3), dtype=np.float64)
    return {
        "quat": quat[: len(imu["accel_x"]), None, :],
        "acc": np.stack([imu["accel_x"], imu["accel_y"], imu["accel_z"]], axis=1).astype(np.float64)[:, None, :],
        "gyro": np.stack([imu["gyro_x"], imu["gyro_y"], imu["gyro_z"]], axis=1).astype(np.float64)[:, None, :],
        "mag": zeros_mag[:, None, :],
    }
