...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the HuMoGen-origin TotalCapture workflow end-to-end.")
    ...
    for sensor_name in [args.sensor_name]:
        ...
        run_command(
            [
                ...
                str(plot_png),
                "--title",
                f"HuMoGen_origin {sensor_name}: raw vs generated",
                "--plot-python",
                args.generator_python,
            ]
        )
...
def build_report(output_root: Path, seed: int, sensor_metrics: dict[str, dict[str, object]]) -> str:
    ...
    lines = [
        "# HuMoGen TotalCapture 当前运行报告",
        "",
        "这份报告描述并行的 HuMoGen 方法：",
        ...
        "3. 从关节运动里提取右手手腕传感器轨迹。",
        "4. 调用 HuMoGen 的 `synthesize_imu(...)` 方法生成 IMU。",
        "5. 因为 HuMoGen 这条方法没有原生 magnetometer 输出，所以补齐 `mag_x/y/z = 0` 以适配当前评测接口。",
