...
## ⚠️ 更正（2026-07-07 数据翻倍对照）：本 v1 基准上的 mix "+37%" 是数据翻倍假象

**动机**：mix（real + synth）训练在 2N 个窗口上，而 TRTR 只有 N 个（synth 与 real 共享
同一 motion，只多了一条 IMU 流）。原文把 mix 相对 TRTR 的 +37% 直接归给"naive 合成 IMU
的增强价值"，但没有做**同尺寸、零新信息**的对照来隔离"仅仅数据翻倍/训练预算"这一混杂。

**对照**（`mix --train-sources real real`，把真实数据复制一份 → 14706 窗，与 mix 同尺寸
但不含任何新信息，3 seeds）：

| 训练组成 | 窗口数 | R@1 | 相对 TRTR |
|---|---|---|---|
| TRTR real | 7353 | 0.0395 ± 0.0066 | — |
| **mix real+REAL（翻倍、无新信息）** | 14706 | **0.0551 ± 0.0117** | **+40%** |
| mix real+naive | 14706 | 0.0541 ± 0.0045 | +37% |
| mix real+globalpose | 14706 | 0.0385 ± 0.0061 | −2% |

**结论：在本 v1（干净真值骨架）基准上，光是复制真实数据就给 +40%，≥ naive 的 +37%——
即 F4/交接建议里"real+naive +37% = 合成 IMU 增强有价值"这个招牌数字，在 v1 上其实几乎
全部来自数据翻倍/训练预算，naive 没有可辨识的额外贡献。** 本 v1 的 mix 增益数字不应再被
引用为"合成 IMU 增强有效"的证据。

**好消息在 estskel（忠实基准）**：同一 real+real 对照在估计骨架下 = 0.0305（**−16%，翻倍
反而伤**），而 real+naive = +44%、real+globalpose = +27%——那里的增益是**真信号**（详见
`sim2real_findings_estskel.md` 的同名更正节）。**教训：mix 增益只能用 estskel 基准说话，
v1 基准因干净骨架下"更多数据即涨"而无法隔离合成信号的价值。**
