...
## ⚠️ 更正（2026-07-07 数据翻倍对照）：本 v1 基准的 mix 数字太噪、不可引用（此前"翻倍假象"说法已撤回）

**动机**：mix（real + synth）训练在 2N 个窗口上，而 TRTR 只有 N 个（synth 与 real 共享
同一 motion，只多了一条 IMU 流）。原文把 mix 相对 TRTR 的 +37% 直接归给"naive 合成 IMU
的增强价值"，但没有做同尺寸对照隔离"仅仅数据翻倍/训练预算"。为此补 `mix --train-sources
real real`（真实数据复制一份 → 14706 窗，同尺寸、无新信息，3 seeds）。

| 训练组成 | 窗口数 | R@1（mean±std, n=3） | 每-seed |
|---|---|---|---|
| TRTR real | 7353 | 0.0395 ± 0.0066 | 0.0408 / 0.0468 / 0.0308 |
| mix real+REAL（翻倍、无新信息） | 14706 | 0.0551 ± 0.0117 | 0.0478 / **0.0716** / 0.0458 |
| mix real+naive | 14706 | 0.0541 ± 0.0045 | 0.0537 / 0.0488 / 0.0597 |
| mix real+globalpose | 14706 | 0.0385 ± 0.0061 | 0.0428 / 0.0428 / 0.0299 |

**结论（校准后）：v1 的 mix 数字太噪、控制之间不自洽，无法干净归因——既不能证明 naive
有效，也不能证明其增益纯属翻倍。** 具体：real+real 名义 +40% 但主要由单个 seed（0.0716）
撑起（std 0.0117）；而**同为 2N 的 real+globalpose 却是 −2%**——若"翻倍即涨"成立，
globalpose 翻倍应同样致涨，故该假设在 v1 内部并不成立。绝对值（0.03–0.07）本就贴近噪声。
**此前"v1 +37% 是数据翻倍假象"是过度声称，予以撤回。** 操作结论：**v1 的 mix 增益一律不
作为证据引用，以 estskel 为准**（下条）。另注：real+real 作为控制本身带 InfoNCE 重复样本
冲突混杂，更干净的应是 real+抖动real；本仓库尚未跑，n=3 亦偏薄。

**estskel（忠实基准）上结论不同且更稳**：同一 real+real 对照在估计骨架下逐 seed 均 ≤ TRTR，
而 real+naive 逐 seed 均高于 TRTR 与 real+real（区间不重叠），且无重复冲突的 real+globalpose
并未复制 naive 的优势——故 estskel 上 naive 的增益是真信号（详见 `sim2real_findings_estskel.md`）。
...
## ⚠️ 更正（2026-07-07 第二轮审查）：逐序列对齐修复后的复跑对照（结论鲁棒）

审查发现语料 real↔motion 的全局尾对齐在部分序列上偏 1–3 帧（逐序列 lag 见
`outputs/sim2real/alignment/lag_table.md`；修复 commit 95251da）。用 lag 口径重建
windows 后全矩阵复跑（`outputs/sim2real_lagfix/tc_rlowarm_w24_v1/`，3 seeds，样本标准差）：
TRTR 0.0395→0.0607±0.0176；TSTR naive 0.0226→0.0212±0.0015，humogen/globalpose 仍≈随机线
——**生成器排名结论不变**；打乱对照 0.0020≈随机线。mix 各列（real+naive 0.0597、
real+globalpose 0.0531、real+real 0.0488）与 TRTR 0.0607 在噪声内不可分——**v1 的 mix
增益不可引用的判定继续成立**。本文其余小节的绝对数为修复前口径。
