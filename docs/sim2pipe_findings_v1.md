# sim2pipe 发现 v1（pipe_probe_tc_v1）

> 数据：`outputs/sim2pipe/pipe_probe_tc_v1/{results.jsonl,report.md}`（21 格，3 seeds）。
> 测试床：TotalCapture，冻结切分 `totalcapture_subject_v1`（S1–S3 训 / S4 验 / S5 测），
> 估计骨架（estskel）motion 侧，传感器 R_LowArm，窗口 w24。
> 裁判：主项目 `IMUVideoMatcher`（冻结 MotionBERT + 冻结 DeSPITE + 训练对齐头，SymmetricInfoNCE），
> 经 subprocess 在主项目 autism_test 环境跑其自带 `slice / train / eval`。指标 = 真实测试窗口的
> batch 内检索 top1（batch 64，理论随机线 0.0156）。

## ⚠️ 更正（2026-07-07 负控审计）：本报告的"打乱对照"无效，"低于随机线/低于地板"两个判据无支撑

审计补跑打乱对照并记到 S5 test（`outputs/sim2pipe_shuffle_floor/`，real 流 3 seeds），
结果 **test_top1 = 0.2524 ± 0.016 ≈ TRTR 0.2518，根本没有掉到本报告"健全性锚点"节声称的
~0.11 地板**（那一行从来没进过账本）。查主项目代码定位原因：

- 主项目 `src/engine/train.py:502` 的 `--shuffle_video_in_batch` 打乱 batch 内 video 后，
  **同时用 `labels_a=perm_inv / labels_b=perm` 把检索 target 一并重标记**——正确的
  IMU↔video 配对被完整保留，它只是"打散批内位置、消除位置偏置"的控制，**不是配对破坏**。
  所以它训练/测试都≈正常 TRTR，本就不该产生地板。
- 对照本仓库 sim2real probe 的 `shuffle_pairs`（`src/sim2real/probe/train.py:49`：真正
  置换 `x_mot` 且**不改** target），那才是配对破坏，掉到 R@1=0.001=随机线。**两者不是
  同一种控制**，本报告误把前者当成了后者。

**因此需要撤回两个判据**：

1. **"三个生成器 TSTR 全塌到随机线附近（< 0.0156）"**——随机线 1/64 也站不住：eval 是
   batch 内对角检索且 `shuffle=False`（`eval.py:84` + `matchers/losses.py:99`），一个 batch
   的 64 个负样本是**同序列时间相邻、重叠的近重复窗口**，真实机会水平不是 1/64、且随
   batch 组成漂移。"低于随机线"这个精确表述不成立。
2. **"打乱对照确认有效地板 ~0.11"**——该控制无效，此 ~0.11 地板无实验支撑。
   （合法负控已补：真实地板 = 0.0201±0.0034，见下"健全性锚点"节与文末对照；
   用此正确地板，"TSTR 崩到地板"反而成立。）

**幸存的稳健结论（不受影响）**：**TSTR test_top1 ≈ 0.01 ≪ TRTR ≈ 0.25**（约 25 倍差距，
远大于 seed 方差）——**合成 IMU 在真实 pipeline 下不迁移**这一主结论、以及归一化消融的
"崩溃不是归一化域移、是信号语义/坐标系硬 gap"结论，都只依赖 TSTR≪TRTR 的**相对**对比，
仍然成立。要撤回的只是"低于随机线/低于地板"这类**绝对判据**的措辞。

**待补 → 已补（2026-07-07）**：以补丁形式（`mainproj_patches/destroy_pairing_in_batch.patch`，
按需套用主项目、用完还原、不污染其历史/远端）给主项目 `train.py` 增加真正的配对破坏 flag
`--destroy_pairing_in_batch`（打乱 video 但**不重标记** target，训练在错配对上；对照
`--shuffle_video_in_batch` 的重标记版）。经 sim2pipe 驱动 `--destroy-pairing` 跑 real 流
3 seeds（`outputs/sim2pipe_true_floor/`）：训练期 train_top1 钉在 0.0155≈随机（对照
shuffle 版爬到 0.82），确认配对确被破坏；**held-out test 真实地板 = 0.0201 ± 0.0034**。

**用正确地板重述结论（比原文更强、且现在有支撑）**：

| 量 | test_top1 | 相对地板 |
|---|---|---|
| TRTR real（锚点） | 0.2518 | ≈12.5× 地板 |
| **真实配对破坏地板** | **0.0201 ± 0.0034** | — |
| TSTR naive / humogen / globalpose | 0.0098–0.0128 | **≈地板或略低** |

- **锚点健康**：TRTR 0.25 ≫ 真实地板 0.02，测试床确实能学到真对齐（原文此结论幸存，
  且现在有合法地板背书，不再依赖失效的 shuffle 控制或理论 1/64）。
- **TSTR 崩到地板成立（但地板是 ~0.02，不是原文的 ~0.11）**：三生成器 TSTR ≈ 0.011，
  落在配对破坏地板（0.020）上、甚至略低——**即合成训练的对齐头，迁移到真实测试的效果
  不优于（甚至略差于）在错配对上训练的头**。这是"合成 IMU 在真实 pipeline 下零迁移"
  最强的表述，现已被合法负控坐实。

## 头版结论

**probe 的排名没有在真实 pipeline 幸存，且合成数据在真实模型下当前无迁移价值。**

| 协议 | 流 | test_top1（S5 真实） |
|---|---|---|
| TRTR | real | **0.2518 ± 0.0055** |
| TSTR | globalpose_clean | 0.0098 ± 0.0020 |
| TSTR | humogen | 0.0128 ± 0.0016 |
| TSTR | naive | 0.0127 ± 0.0020 |
| mix | real+globalpose_clean | 0.2601 ± 0.0010 |
| mix | real+humogen | 0.2374 ± 0.0102 |
| mix | real+naive | 0.2406 ± 0.0136 |

三条主线：

1. **TRTR 强、锚点健康**：0.2518，约 16× 理论随机线，远高于冻结编码器的零样本地板
   （打乱对照把 val 压在 ~0.11 不涨，见下）。测试床成立。
2. **三个生成器 TSTR 全部塌到随机线附近**（0.010–0.013，彼此差 0.003 < 随机线 0.0156，
   统计上不可区分）。这与 sim2real probe 完全相反——那里 naive TSTR R@1=0.0172（17× chance）
   明显胜出、humogen/globalpose 仅 2× chance。**真实模型下三者都失败，naive 的优势是 probe 的假象。**
3. **mix 无增益**：real+synth ≈ TRTR（naive/humogen 甚至略降，globalpose 持平在噪声内）。
   sim2real probe 里 mix real+naive 相对 TRTR +141%——**这个增益也没幸存。**

## 为什么 probe 高估：量具不同

sim2real probe 从零训练自己的小 IMU 塔，学的是"这批合成信号内部的可分性"；只要合成流自洽，
probe R@1 就非零，于是给出乐观排名。sim2pipe 的 IMU 塔是**冻结的预训练 DeSPITE**（在真实 IMU 统计上训得），
只训一个薄对齐头：
- TSTR 用合成流训头 + 用**合成源的归一化统计量**施加到真实测试流（诚实 TSTR 部署假设）。合成与真实
  的帧语义/尺度一旦不匹配，真实测试 IMU 经合成统计归一化后落到 DeSPITE 没见过的区域 → **低于随机**。
- 这不是 bug，是**真实 pipeline 层面的 sim-to-real gap 被如实测出**：当前三个生成器产出的信号，
  对一个在真实 IMU 上预训练的编码器都不可用。probe 因为自训编码器绕过了这道坎，才显得乐观。

## 健全性锚点（N1 门禁）

- **格式往返** ✅：导出 npz 经主项目 `slice` 无警告物化，37 序列 9207 窗口。
- **真实锚点** ✅：TRTR 0.25 ≫ 随机线，≫ 零样本地板。
- **打乱对照** ✅：`--shuffle_video_in_batch` 训练下 val_top1 被压在 ~0.10–0.11 不随 epoch 上升
  （正常训练 train_top1 持续爬升）——确认这个测试床的**有效地板是冻结预训练编码器的零样本对齐 ~0.11**，
  而非理论 1/64。
  > ⚠️ **已撤回并更换（见顶部更正节）**：该 flag 会重标记 target、保留正确配对，不是配对破坏负控；
  > 审计复跑 3 seeds 得 test≈0.25≈TRTR，非 ~0.11。合法负控 `--destroy_pairing_in_batch` 给出的
  > **真实地板 = 0.0201±0.0034**；TSTR≈0.011 落在此地板上，"崩到地板"用正确地板成立。

## 对主项目 G4 的直接含义

- **I4（数据增强）/ I5（预训练）用当前三个生成器都不会奏效**：pipe 层面 TSTR 崩、mix 不涨。
  在把合成数据接进 custom 单 IMU 目标之前，生成器必须先跨过"匹配真实 IMU 统计/帧语义"这道坎。
- **首要修复方向 = F2（globalpose 比力输出变体）**：DeSPITE 期望比力（含重力）语义；naive 输出自由加速度、
  humogen 缺重力对齐。F2 出来后**优先进 sim2pipe 复跑**，这是唯一能翻盘 TSTR 的已知杠杆。
- 次要：TSTR 归一化鲁棒性（用真实源统计而非合成源统计做消融，隔离"归一化域移"与"信号语义差"两个因素）。

## 局限

- 单一测试床（TotalCapture）、单窗口（w24）、单指标（batch 内 top1）。绝对数受 batch 大小影响。
- estskel 退化参数仍是文献默认值（继承 sim2real 局限），未用真实 AlphaPose 标定。
- P2（合成预训练 → custom 微调 → FrameAcc）本期未做，另行立项；但 P1 已强烈提示 P2 用当前生成器不会有正收益。

## 归一化消融：拆开"域移"与"信号语义"（2026-07-07）

**问题**：TSTR 崩到随机线以下，是因为 (a) 用合成源统计量归一化、真实测试 IMU 被推到 DeSPITE 分布外
（**归一化域移**），还是 (b) 合成信号本身的坐标系/帧语义和真实不同（**硬 sim-to-real gap**）？

**消融**（`outputs/sim2pipe_normabl/`，9 格 TSTR × 3 seeds）：用 **oracle 真实统计量**（TRTR 的 S1–S3 真实
`imu_stats.json`）归一化 train+test，其余不变。测试侧真实 IMU 因此和 TRTR 完全同样处理——若合成训练的
对齐头能对齐真实 IMU 嵌入，就该得分。

| 流 | synth-stats（baseline） | real-stats（oracle） | Δ |
|---|---|---|---|
| naive | 0.0127 ± 0.0020 | 0.0098 ± 0.0020 | −0.0029 |
| humogen | 0.0128 ± 0.0016 | 0.0122 ± 0.0011 | −0.0006 |
| globalpose_clean | 0.0098 ± 0.0020 | 0.0105 ± 0.0024 | +0.0006 |

**结论：三个生成器用完美真实统计量归一化后 TSTR 全都没恢复**（Δ 全在噪声内，仍钉在随机线）。
对照 mix 在 real-stats 下仍 0.2455（真实统计量本身没破坏管线）。**所以崩溃的主因不是归一化域移，
是信号的坐标系/帧语义结构差**——这个差是"每帧向量被一个旋转/反射作用"，减逐通道均值（一阶统计量）
根本抵消不了，所以 oracle 消融救不回来，逻辑闭环。

### 三个生成器各自差在哪（S1_acting1 代表序列，加速度逐轴 vs 幅值）

| 源 | 逐轴相关 x/y/z | 幅值‖acc‖相关 | acc 均值 | ‖acc‖均值 | 病因 |
|---|---|---|---|---|---|
| real | — | — | [7.4, −4.7, 2.1] | 11.2 | 基准（含重力 DC） |
| naive | **−0.92**/+0.61/+0.38 | **+0.89** | [−6.4, −1.5, 2.3] | 11.8 | **有重力、量级对，但坐标系/符号约定反**（x 轴符号相反） |
| globalpose_clean | −0.16/−0.44/+0.24 | +0.75 | [0, 0, 0] | 5.6 | **缺重力 DC**（零均值、幅值偏小） |
| humogen | −0.43/+0.29/+0.21 | +0.50 | [−1.5, 1.0, 1.5] | 12.5 | **重力轴约定错**（部分） |

**关键更正：naive 不是"没有重力"——它有重力、幅值和真实几乎一致（11.8 vs 11.2）、幅值相关高达 0.89，
只是坐标系/符号约定和真实 Xsens 不一致（x 轴逐轴相关 −0.92，整轴反号）。** 三个生成器病因各异，
但在 pipe 下 TSTR 全崩，说明冻结的真实编码器对**任何**这类语义不一致都不容忍。

### 为什么信号级指标与 sim2real probe 都"看不见"这个差，pipe 却崩

| 量具 | 它看什么 | 对坐标系/符号差异 |
|---|---|---|
| L0 门禁 / correlation | 主要看**幅值** ‖acc‖ | **免疫**——幅值是旋转不变量，整轴反号 ‖acc‖ 不变（naive 幅值相关 0.89 照样过） |
| sim2real probe | 自建、**从零训练**的 IMU 塔 | **免疫**——就地适配合成数据的坐标系约定，只要内部自洽、波形可分即可（naive 达 TRTR 的 57%） |
| sim2pipe（你的 DeSPITE） | **在真实 Xsens 坐标系上预训练、冻结**的 IMU 塔 | **敏感**——铁按真实约定解读输入，naive 的反号轴 → 嵌入错乱 → TSTR 训测嵌入空间对不上 → 崩 |

这解释了表面矛盾：**correlation 好、sim2real 好、一到 pipe 就崩，并不冲突**——前两个量具对"整体坐标系/符号
差异"天然免疫（一个看幅值、一个自训编码器就地适配），只有 sim2pipe 用了一个锁死在真实坐标系上的编码器，
才把这个语义差暴露成迁移失败。也正因此，**这个 gap 必须用 pipe 级评测才发现得了**。

**下一步（精确化）**：修复必须在**信号生成层面**，且目标不是笼统"加重力"，而是**把合成 IMU 对齐到真实
Xsens 的坐标系与比力约定**（轴向、符号、比力 vs 自由加速度、重力投影）。我方计划的 globalpose 比力输出
变体正是这个方向（内部记作 sim2real 发现 F2 的后续验证）。归一化鲁棒性不是杠杆。

## ⚠️ 更正（2026-07-07 第二轮审查）：逐序列对齐修复后的基准数（P0-1）

第二轮代码审查发现 `src/sim2pipe/export.py` 用**头对齐**截断 IMU↔骨架，而语料中
real 与 motion 的正确偏移是逐序列的（`imu_motion_lag`，实测范围 [-3, +44]）。
最严重的 S5_freestyle3（**test 序列**，占 test 窗口约 22%）real 比 motion 短 44 帧：
lag 扫描显示尾对齐（lag=44）acc 幅值相关 0.94、头对齐 0.02——即此前**所有** pipe 结果
中该序列逐窗错配 0.73s。修复：`01c_estimate_alignment.py` 逐序列估 lag 写入 meta.json，
export/windows/gate 统一消费（commit 95251da）。重跑核心格（新输出根
`outputs/sim2pipe_lagfix/`、`outputs/sim2pipe_true_floor_lagfix/`，3 seeds，样本标准差）：

| 格 | 修复前 | 修复后 | 变化 |
|---|---|---|---|
| TRTR real | 0.2518±0.0067 | **0.2976±0.0105** | **+18%（S5_freestyle3 修复的量级）** |
| 真配对破坏地板 | 0.0201±0.0041 | 0.0208±0.0027 | 不变（地板本就无配对） |
| TSTR naive | 0.0127±0.0024 | 0.0072±0.0023 | 仍≈地板（略降） |
| mix real+naive | 0.2406±0.0167 | 0.2934±0.0193 | ≈TRTR，仍无增益 |

**所有定性结论幸存且更干净**：TRTR(0.2976) ≫ 地板(0.0208)；TSTR naive 钉在地板上
= 零迁移；mix 无增益。本文其余小节的绝对数（TRTR 0.2518 等）均为修复前口径，引用时
以本节为准；F2 变体一律与本节的修复后基线比较。

附带发现：S1_acting2（train 序列）在任何 lag 下 real↔合成 acc 相关峰值仅 0.12
（其余序列 0.39–0.98），疑似该序列 real↔AMASS 配对本身有问题，待查（详见
`outputs/sim2real/alignment/lag_table.md`）。

## 📐 判定口径切换(2026-07-07 定):全库检索取代主项目原生批内 top1

决定:**评测判定不再沿用主项目的原生 eval**(批内 top1,batch=64 连续重叠窗口、
双向平均——随机线依赖 batch 大小、负样本是同视频近重复,协议脆弱且不可外比)。
自本节起 sim2pipe 的判定与 sim2real 完全对齐:**主项目模型只当编码器**,判定 =
IMU→motion 单向、全 test 库(1005 窗,与 sim2real test__real 同一窗口网格)检索
R@1/R@5,chance = 1/1005 ≈ 0.001。实现:`scripts/sim2pipe/04_eval_fullgallery.py`
(对已训 checkpoint 直接重判,不重训) + `src/sim2pipe/main_env_scripts/embed_windows.py`。

lagfix 12 个 checkpoint 重判结果(`outputs/sim2pipe_fullgal/`,3 seeds,样本标准差):

| 格 | 原生批内 top1(64选1) | **全库 R@1(1005选1,新口径)** |
|---|---|---|
| TRTR real | 0.2976 | **0.0872±0.0083(87× chance)** |
| 真配对破坏地板 | 0.0208(≈1/64) | **0.0010 = chance(地板落在理论随机线上)** |
| TSTR naive | 0.0072 | **0.0000(精确零迁移)** |
| mix real+naive | 0.2934 | 0.0886±0.0138(≈TRTR,无增益) |

两套 pipeline 自此同一把尺子,可直接同表比较:

| 同口径(全库 R@1,chance 0.001) | sim2real 自建裁判 | sim2pipe 主项目编码器 |
|---|---|---|
| TRTR | 0.0414 | 0.0872 |
| TSTR naive | 0.0156 | 0.0000 |
| mix real+naive | 0.0547(+32%) | 0.0886(≈TRTR) |
| 地板 | 0.0010 | 0.0010 |

读法:主项目编码器在真实数据上是更强的裁判(0.087 vs 0.041),但对合成数据判零;
自建裁判(就地训练、可适应合成坐标系)下 naive 有部分迁移与 mix 增益。两者分歧
本身就是结论:**现有生成器的信号在"固定于真实约定的编码器"下无效——F2 语义修复
仍是前置条件。**旧的批内口径数字保留仅作历史对照,后续一律引用本节口径。
